...
void SimCrossSection::ReadScatteringCrossSection(std::string fileName)
{
...
void SimCrossSection::ParseLine(std::string& line, std::vector<double>& coeffs)
{
    for(int i = 0; i < ENDFLineWidth; i += ENDFNumberWidth)
    {
        std::string number {line.substr(i, ENDFNumberWidth)};
...
void SimCrossSection::ReadAngularCrossSection(std::string fileName)
{
    std::ifstream streamer {};
    streamer.open(fileName.c_str());
    if(!streamer)
...
void SimCrossSection::DrawScattering()
{
...
void SimCrossSection::DrawAngular()
{
...
void SimCrossSection::ComputeXSAtEnergy(double Tn)
{
...
double SimCrossSection::EvalScatteringXS(double Tn)
{
    return functScattering->Eval(Tn);
}

double SimCrossSection::IntegrateTotalXS(double low, double high)
{
    return xsAtEnergy->Integral(low, high);
}

double SimCrossSection::SampleXS(TRandom3* generator)
{
    double cosValue { xsAtEnergy->GetRandom((generator == nullptr) ? gRandom : generator)};
    return TMath::ACos(cosValue);
}

void SimCrossSection::DrawXS()
{
    if(!canvFinalXS)
...
void SimCrossSection::DrawFiles()
{
    DrawScattering();
    DrawAngular();
    gPad->cd();

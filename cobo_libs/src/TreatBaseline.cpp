...
int MEvent::TreatBaseline(bool FullTreeOpt, bool IsFastPeak)
{
          ...
          if(CoAs.Channel[Ch].Sample_Number.size()==NB_SAMPLES)
          {
            for(unsigned short Bu : CoAs.Channel[Ch].Sample_Number)
					  { 
					  	...
					  	if(Bu<50) BL1+=(CoAs.Channel[Ch].Raw_Sample[Bu]-BaselineCal[co][as][ag][ch][Bu])/50.;
						  if(Bu>460) BL2+=(CoAs.Channel[Ch].Raw_Sample[Bu]-BaselineCal[co][as][ag][ch][Bu])/50.;
					  }
...
/////////////////////////////////////////////////
          ...
          for(unsigned short Bu : CoAs.Channel[Ch].Sample_Number)
              ...
              float next2_charge=(CoAs.Channel[Ch].Raw_Sample[Bu+2]-BaselineCal[co][as][ag][ch][Bu+2]-Baseline)*CalibCoefs[co][as][ag][ch][0]+CalibCoefs[co][as][ag][ch][1];
							if(charge>P_CHARGE && charge>THR_PAD && charge > prev1_charge && charge > prev2_charge)
								if(!NMAX || (NMAX && charge> next1_charge && charge> next2_charge))
								{
									P_CHARGE=charge;
									P_TIME=Bu;
									NSUP=0;
							...
							if(charge<P_CHARGE && P_CHARGE>0) NSUP=Bu-P_TIME;
							if(NSUP>=MIN_INT)
							{
								NMAX++;
